...
• Adds unified Messaging for SMS and WhatsApp, with a single Inbox covering your contacts and people who aren't in your CRM yet (Pro)
• Adds one-to-one SMS and WhatsApp conversations with any phone number, before that person becomes a contact (Pro)
• Adds one-time recipients for Messaging campaigns, from a CSV upload or pasted phone numbers (Pro)
• Records every message sent to your business number in the Inbox, whether or not the sender is a contact (Pro)
• Adds WhatsApp template creation, management, and syncing without leaving FluentCRM (Pro)
• Adds reusable SMS message templates for faster replies and campaigns (Pro)
• Receives WhatsApp images and documents in the Inbox (Pro)
• Adds a FluentCart Create Coupon automation action with coupon smartcode support
• Adds theme colors to the email editor
• Adds a Media & Text block to the email editor
```

```markdown [🚀 Improvements]
• Rebuilds Messaging on a conversation-thread architecture, the foundation for further channels and features
• Collapses Messaging settings into a single tabbed screen
...
• Lets non-subscribed contacts be re-invited through opt-in forms and LMS integrations
• Includes active WooCommerce subscriptions when re-syncing
• Adds WordPress 7.1 compatibility
```

```markdown [🐞 Bug fixes]
• Fixes the fluent_crm/default_email_design_template filter not applying to templates created from scratch
• Fixes the activity days-before contact filter
• Honors the configured Default Contact Status for WooCommerce and EDD
• Fixes heading font weight in the email preview
• Fixes raw HTML campaign content not saving in the editor
• Fixes Campaign Archive shortcode parameters being ignored when no ids are given
• Fixes single campaign archive rendering
• Fixes the WooCommerce review smartcode responsive layout
• Fixes https:// being added to smartcode button links
• Fixes image links being lost when replacing an image from the media library
• Fixes styling on the View in Browser email page
• Fixes outdated revenue showing after re-syncing a campaign report
• Fixes bounces from ToSend not being recorded
• Fixes incorrect advanced filter results for numeric contact custom fields
• Fixes phone numbers in local format being rejected
• Fixes tag search returning empty results from page 2 onward
...
```markdown [🔒 Security]
• Strengthens permissions and handling for imports, exports, settings, and external connections
• Adds safeguards against repeated subscription-link email requests
• Validates the target role's capabilities before the Change User Role automation action applies it
• Enforces SSL and safer remote calls in more places
```
